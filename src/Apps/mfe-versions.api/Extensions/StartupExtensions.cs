    ...
    public static class StartupExtensions
    {
        ...
        private static void UseMapControllers(this IApplicationBuilder app)
        {
            app.UseEndpoints(endpoints =>
            {
                ...
                endpoints.MapHealthChecks("/activecheck", new HealthCheckOptions() { Predicate = (check) => check.Tags.Contains("host"), ResponseWriter = ResponseWriters.HostProbeWriter });
                endpoints.MapHealthChecks("/probe/ready", new HealthCheckOptions() { Predicate = (check) => check.Tags.Contains("ready") });
                endpoints.MapHealthChecks("/probe/healthreport", new HealthCheckOptions() { Predicate = (check) => check.Tags.Contains("ready"), ResponseWriter = ResponseWriters.HealthReportWriter });
            });
        }
